...
const { sm3 } = smCrypto as unknown as { sm3: (input: Uint8Array | string | Buffer) => string };
...
class CryptoUtility {
  ...
  constructor(salt: string, customBase64Alphabet: string[]) {
    ...
    this.bigArray = [
      ...
      0, 200, 244, 176, 218, 44, 184, 172, 49, 216, 93, 168, 53, 21, 183, 41, 67, 85, 224, 155,
      226, 242, 87, 177, 146, 70, 190, 12, 162, 19, 137, 114, 25, 165, 163, 192, 23, 59, 9, 94,
...
class BrowserFingerprintGenerator {
  static generateFingerprint(platform: string): string {
    const rand = (min: number, max: number) => Math.floor(Math.random() * (max - min + 1)) + min;
    const innerWidth = rand(1366, 1920);
    const innerHeight = rand(768, 1080);
    const outerWidth = innerWidth + rand(16, 32);
    const outerHeight = innerHeight + rand(72, 96);
    const screenX = 0;
    const screenY = 0;
    const sizeWidth = rand(1366, 1920);
    const sizeHeight = rand(768, 1080);
    const availWidth = sizeWidth;
    const availHeight = sizeHeight - rand(32, 48);
...
export class ABogus {
  ...
  private aid: number;
  private uaKey: number[];
  private sortIndex: number[];
  private sortIndex2: number[];
  ...
  constructor(fp?: string, userAgent?: string, options?: number[]) {
    ...
    this.browserFp = fp && fp.length > 0 ? fp : BrowserFingerprintGenerator.generateFingerprint("Win32");
